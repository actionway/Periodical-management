...
<h1><center>期刊借阅表</center></h1>
		...
		<table border="2" id="overflowTest">
			...
			<%
				Statement stmt = null;
					ResultSet rs = null;
					String sql = "SELECT * FROM borr;"; //查询语句
			...
			<tr id="er">
				...
				<td><%=rs.getString("Usernum")%></td>
				<td><%=rs.getString("Username")%></td>
				<td><%=rs.getString("Journ")%></td>
				<td><%=rs.getString("Year")%></td>
				<td><%=rs.getString("Roll")%></td>
				<td><%=rs.getString("Duty")%></td>
				<td><%=rs.getString("Borrowday")%></td>
				<td><%=rs.getString("Returnday")%></td>
			...
			<button onclick="location='borradd.jsp'" class="button button1">添加</button>
			<button onclick="location='borrdelete.jsp'" class="button button1">删除</button>
			<br/>
			<br/>
			<form action="borrselect.jsp" method="post" name="re">
